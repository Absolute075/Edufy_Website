...
export default function BirdMigrationPage() {
  ...
  const isTextAnswerKey = (key: QuestionKey) =>
    key === "q23" || key === "q24" || key === "q25" || key === "q26";

  const isMultiChoicePairKey = (key: QuestionKey) => key === "q21" || key === "q22";

  const isMultiChoicePairCorrect = (aRaw: string, bRaw: string) => {
    const a = (aRaw ?? "").trim();
    const b = (bRaw ?? "").trim();
    if (!a || !b) return false;
    const user = [a, b].sort().join(",");
    const correct = [correctAnswers.q21, correctAnswers.q22].sort().join(",");
    return user === correct;
  };
  ...
  const handleSubmit = () => {
    ...
    let s = 0;
    let multiChoicePairHandled = false;
    (Object.keys(correctAnswers) as QuestionKey[]).forEach((key) => {
      if (isMultiChoicePairKey(key)) {
        if (multiChoicePairHandled) return;
        multiChoicePairHandled = true;
        if (isMultiChoicePairCorrect(selections.q21, selections.q22)) s += 2;
        return;
      }
  ...
  useEffect(() => {
   ...
   const answerSheet = useMemo(() => {
     return progressNumbers.map((num) => {
       ...
       const isCorrect =
         userDisplay !== "N/A" &&
         (isMultiChoicePairKey(key)
           ? isMultiChoicePairCorrect(selections.q21, selections.q22)
           : isTextAnswerKey(key)
  ...
  const toggleMultiChoicePairOption = (opt: string) => {
    if (submitted) return;
    setSelections((s) => {
      const a = (s.q21 ?? "").trim();
      const b = (s.q22 ?? "").trim();
      ...
      if (b === opt) return { ...s, q22: "" };
      if (!a) return { ...s, q21: opt };
      if (!b) return { ...s, q22: opt };
      return { ...s, q22: opt };
    });
  };
  ...
  <div>
    <Head>
      ...
      <title>Reading Passage 2 – Bird Migration</title>
      <link
        rel="stylesheet"
       ...
       >
         ...
         >
           <div className="passage-col" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
             ...
             >
              <h3 style={{ marginTop: 0, marginBottom: 16, fontSize: 20, fontWeight: 700 }}>Bird Migration</h3>
              ...
              {passageHtml === null ? (
                <div id="passage2" ref={passageRef}>
                  <p>
                    ...
                    food is plentiful, their bodies can accumulate considerable layers of fat, to provide sufficient
                  ...
                  <p>
                    ...
                    it is in short supply. This particularly applies to birds that breed in the temperate and Arctic
                    regions of the Northern Hemisphere, where food is abundant during the short growing season. Many
                    species can tolerate cold temperatures if food is plentiful, but when food is not available,
                  ...
                  <p>
                    ...
                    food and good weather. Nobody knows, for instance, why British swallows, which could presumably
                    survive equally well if they spent the winter in equatorial Africa, instead of flying several
                    thousands of miles further to their preferred winter home in South Africa’s Cape Province.
                    Another mystery involves the huge migrations performed by Arctic terns and mudflat-feeding
                    shorebirds that breed close to the Polar Regions. In general, the further north a migrant
                    species breeds, the further south it spends the winter. For Arctic terns, this necessitates an
                    annual round trip of 25,000 miles. Yet, en route to their final destination in far southern
                    latitudes, all these individuals overfly other areas of seemingly suitable habitat spanning two
                    hemispheres. While we may not fully understand birds’ reasons for going to particular places, we
                  ...
                  <p>
                    ...
                    may even have little or no inkling of their parents’ appearance. A familiar example is that of
                    the cuckoo, which lays its eggs in another species’ nest and never reencounters its young. It is
                    mind-boggling to consider that, once raised by its host species, the young cuckoo makes its way
                    to ancestral wintering grounds in the tropics before returning single-handedly to northern
                    ...
                    mental image of what another cuckoo looks like. Yet nobody has the slightest idea as to how this
                    is possible.
                  </p>
                  <p>
                    <span className="label">E</span>
                    Mounting evidence has confirmed that birds use the positions of the sun and stars to obtain
                    compass directions. They also seem to be able to detect the earth’s magnetic field, probably due
                    to having minute crystals of magnetite in the region of their brain. However, accurate
                    navigation also requires an awareness of position and time, especially when lost. Experiments
                    have shown that after being taken thousands of miles over an unfamiliar land mass, birds are
                    ...
                    the earth’s magnetic field. How the birds use their ‘instruments’ remains unknown, but one thing
                    is clear: they see the world with a superior sensory perception to ours. Most small birds
                    ...
                    which calibrates their compass. Traveling at night provides other benefits. Daytime predators
                    are avoided and the danger of dehydration due to flying for long periods in warm, sunlit skies
                    is reduced. Furthermore, at night the air is generally cool and less turbulent and so conducive
                    to sustained, stable flight.
                  </p>
                  <p>
                    ...
                    favorable winds. Birds are adept at both, and, in laboratory tests, some have been shown to
                    detect the minute difference in barometric pressure between the floor and ceiling of a room.
                    Often they react to weather changes before there is any visible sign of them. Lapwings, which
                    feed on grassland, flee west from the Netherlands to the British Isles, France, and Spain at the
                    onset of a cold snap. When the ground surface freezes, the birds could starve. Yet they return
                    to Holland ahead of a thaw, their arrival linked to a pressure change presaging an improvement
                    in the weather.
                  </p>
                  <p>
                    ...
                    fast-moving westerly tailwinds. Not only do they arrive safely in Europe, but, based on ringing
            ...
            >   
              ...
              <div className="question-group">
                ...
                <p>
                  ...
                  <br />
                  Write the correct number, <strong>i–x</strong>, in boxes <strong>14–20</strong> on your answer sheet.
                </p>

                <p style={{ marginTop: 12 }}>
                  <strong>List of Headings</strong>
                  <br />
                  <strong>i</strong> The best moment to migrate
                  <br />
                  <strong>ii</strong> The unexplained rejection of closer feeding ground
                  <br />
                  <strong>iii</strong> The influence of weather on the migration route
                  <br />
                  <strong>iv</strong> Physical characteristics that allow birds to migrate
                  <br />
                  <strong>v</strong> The main reason why birds migrate
                  <br />
                  <strong>vi</strong> The best wintering grounds for birds
                  <br />
                  <strong>vii</strong> Research findings on how birds migrate
                  <br />
                  <strong>viii</strong> Successful migration despite the problem of wind
                  <br />
                  <strong>ix</strong> The contrast between long-distance migration and short-distance migration
                  <br />
                  <strong>x</strong> Mysterious migration despite lack of teaching
                </p>

                <div className="question question--plain" data-question="14">
                  <div className="question-text">Paragraph A</div>
                  {renderLetterDropdown("q14", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="15">
                  <div className="question-text">Paragraph B</div>
                  {renderLetterDropdown("q15", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="16">
                  <div className="question-text">Paragraph C</div>
                  {renderLetterDropdown("q16", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="17">
                  <div className="question-text">Paragraph D</div>
                  {renderLetterDropdown("q17", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="18">
                  <div className="question-text">Paragraph E</div>
                  {renderLetterDropdown("q18", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="19">
                  <div className="question-text">Paragraph F</div>
                  {renderLetterDropdown("q19", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>

                <div className="question question--plain" data-question="20">
                  <div className="question-text">Paragraph G</div>
                  {renderLetterDropdown("q20", ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"])}
                </div>
              </div>

              <div className="question-group" style={{ marginTop: 28 }}>
                <div className="question-group-title"><strong>Questions 21 and 22</strong></div>
                <p>
                  Choose <strong>TWO</strong> letters, <strong>A–E</strong>.
                  <br />
                  Write the correct letters in boxes <strong>21 and 22</strong> on your answer sheet.
                  <br />
                  Which <strong>TWO</strong> of the following statements are true about bird migration?
                </p>

                <div className="question question--plain" data-question="21" style={{ marginTop: 12 }}>
                  <div className="radio-group">
                    {(
                      [
                        { value: "A", label: "Birds often fly further than they need to." },
                        { value: "B", label: "Birds traveling in family groups are safe." },
                        { value: "C", label: "Birds flying at night need less water." },
                        { value: "D", label: "Birds have much sharper eyesight than humans." },
                        { value: "E", label: "Only shorebirds are resistant to strong winds." },
                      ] as Array<{ value: string; label: string }>
                    ).map((opt) => {
                      const a = (selections.q21 ?? "").trim();
                      const b = (selections.q22 ?? "").trim();
                      const checked = a === opt.value || b === opt.value;
                      return (
                        <label key={opt.value} className="radio-option" style={{ alignItems: "center" }}>
                          <input
                            type="checkbox"
                            className="radio-input"
                            checked={checked}
                            disabled={submitted}
                            onChange={() => toggleMultiChoicePairOption(opt.value)}
                          />
                          <span className="radio-circle" aria-hidden="true" />
                          <span className="radio-text">
                            <span className="radio-letter">{opt.value}.</span>
                            {opt.label}
                          </span>
                        </label>
                      );
              ...
              <div className="question-group" style={{ marginTop: 28 }}>
                ...
                <p>
                  ...
                  <br />
                  Choose <strong>NO MORE THAN ONE WORD OR A NUMBER</strong> from the passage for each answer.
                </p>
                ...
                <div className="question question--plain" data-question="23">
                  <div className="question-text" style={{ fontWeight: 400 }}>
                   It is a great mystery that young birds like cuckoos can find their wintering grounds without
                    {renderTextInput("q23")} assistance.
                  </div>
                </div>

                <div className="question question--plain" data-question="24">
                  <div className="question-text" style={{ fontWeight: 400 }}>
                    Evidence shows birds can tell {renderTextInput("q24")} like a compass by observing the sun and the stars.
                  </div>
                </div>

                <div className="question question--plain" data-question="25">
                  <div className="question-text" style={{ fontWeight: 400 }}>
                    One advantage for birds flying at night is that they can avoid contact with {renderTextInput("q25")}.
                  </div>
                </div>

                <div className="question question--plain" data-question="26">
                  <div className="question-text" style={{ fontWeight: 400 }}>
                    Laboratory tests show that birds can detect weather without {renderTextInput("q26")} signs.
                  </div>
                </div>
              </div>
            </div>
