...
export default function Reading349653Page() {
  ...
  const answerSheet = useMemo(() => {
    return progressNumbers.map((num) => {
      ...
      const userRaw = (selections[key] ?? "").trim();
      const correct = correctAnswers[key];
      const isCorrect = Boolean(userRaw) && userRaw === correct;
      return {
        num,
        userDisplay: userRaw || "N/A",
        correct,
        isCorrect,
      };
    });
  ...
  const summaryWordOptions = [
    { value: "A", label: "A - archive" },
    { value: "B", label: "B - areas" },
    { value: "C", label: "C - accessible" },
    { value: "D", label: "D - functions" },
    { value: "E", label: "E - features" },
    { value: "F", label: "F - social" },
    { value: "G", label: "G - relevant" },
    { value: "H", label: "H - political" },
    { value: "I", label: "I - up to date" },
    { value: "J", label: "J - web" },
    { value: "K", label: "K - non-authorised" },
  ];

  const q32Options = [
    { value: "A", text: "The type of information which is communicated" },
    { value: "B", text: "The people from whom the information is sent" },
    { value: "C", text: "The amount of information which is communicated" },
    { value: "D", text: "The channels by which information is communicated" },
  ];

  const q33Options = [
    { value: "A", text: "Part of the Library of Congress" },
    { value: "B", text: "Trying to save all types of media" },
    { value: "C", text: "Involved in preserving internet data" },
    { value: "D", text: "Trying to retrieve pre-1996 data" },
  ];

  const q34Options = [
    { value: "A", text: "Avoid the loss of digital data" },
    { value: "B", text: "Preserve old documents and photographs" },
    { value: "C", text: "Identify important contents" },
    { value: "D", text: "Teach the importance of preservation" },
  ];

  const q35Options = [
    { value: "A", text: "Storing archives on CDs" },
    { value: "B", text: "Investigating the durability of CDs" },
    { value: "C", text: "Restoring information from damaged CDs" },
    { value: "D", text: "Duplicating information from old CDs" },
  ];

  const q36Options = [
    { value: "A", text: "Original copies of some web materials" },
    { value: "B", text: "Factually accurate information about the web" },
    { value: "C", text: "Sufficiently academic information about the web" },
    { value: "D", text: "Reconstructed versions of web materials" },
  ];
  ...
  const dropdownLabel = (key: QuestionKey, value: string) => {
    ...
    if (key === "q37" || key === "q38" || key === "q39" || key === "q40") {
  ...
  const renderDropdown = (key: QuestionKey, options: Array<{ value: string; label: string }>, className?: string) => {
    ...
    const isDropUp = key === "q37" || key === "q38" || key === "q39" || key === "q40";
    return (
      <div className={`rounded-dropdown${className ? ` ${className}` : ""}`}>
        ...
        <div
          className={`rounded-dropdown-menu${isOpen ? " is-open" : ""}${isDropUp ? " is-up" : ""}`}
          role="listbox"
          aria-hidden={!isOpen}
        >
          {options.map((o) => (
  ...
  const renderRadioGroup = (key: QuestionKey, options: Array<{ value: string; text: string }>) => {
    const current = selections[key] ?? "";
    return (
          ...
          return (
            ...
            >
              ...
              <span className="radio-text">
                <span className="radio-letter">{o.value}</span>
                {o.text}
              </span>
            </label>
          );
  ...
  return (
    <div>
      <Head>
        ...
        <title>Reading Passage 3 – Recording History</title>
      ...
      >
                    ...
                    <div key={a.num} className="answer-item">
                      <div className="answer-left">
                        ...
                        <div className="answer-line">
                          Answer: <strong style={{ color: "var(--text)" }}>{a.userDisplay}</strong>
                        </div>
                        <div className="answer-line">
                          Correct: <strong style={{ color: "var(--text)" }}>{a.correct}</strong>
                        </div>
                      </div>
        ...
        >
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: "var(--text)" }}>Passage 3</div>
          <div style={{ fontSize: 14, color: "var(--muted)", fontWeight: 700 }}>Read the text and answer questions 27–40</div>
        </div>
        ...
        >
          <div className="passage-col" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
            ...
            >
              <h3 style={{ marginTop: 0, marginBottom: 16, fontSize: 20, fontWeight: 900 }}>Recording History</h3>
              {passageHtml === null ? (
                <div id="passage2" ref={passageRef}>
                  <p>
                    <span className="label">A</span> Not too long ago, America’s culture was recorded mainly on paper, vinyl and film, and deposited at the Library of Congress and the U.S. Copyright Office. Nowadays, the seeds of future history may be flowing in huge volume from only designers and content providers directly into people’s homes, and the Library of Congress and other activities are trying to create new systems to ensure the information is saved. It’s a big job, they say, but also a big opportunity. “We are given an opportunity through the digital medium to create time and disseminate knowledge in ways never possible before, and if we make the wrong steps, we can lose not only this opportunity, but also our cultural heritage that’s in digital form,” says Brewster Kahle, founder of the Internet Archive.
                  </p>
                  <p>
                    <span className="label">B</span> Records, the Internet, Could Be Future Keys to Our Times. Kahle’s Archive is working with the Library of Congress and private industry to preserve a record of the Internet for future generations, and has been saving parts of the Web for five years. Kahle says he knows of “no good archive information collection per-1996” and he feels that it’s a shame. But, he adds, it is not unprecedented in history. “The early version of whatever media is usually lost,” he says. “Early films were recycled for their better content. Any books from the first 30 years of printing demand a very high price, because they rarely exist, and the library of the standard fan attempt to archive collected knowledge of the ancient world in best known for being burned by three successive governments — first the Romans, then the Christians, then the Muslims.”
                  </p>
                  <p>
                    <span className="label">C</span> Planning a Preservation System Still, archivists don’t want to fall behind this time, if they can play it. In December, Congress appropriated $100 million to the Library of Congress to develop a national representative to preserve digital information. The effort is about more than just money, say archiving professionals. “The last stage of this era is to make sure we understand the issues related to long-term preservation,” says Laura Campbell of the Library of Congress’ Digital Information Program. “What are the considerations to making sure this content isn’t lost to future generations?”
                  </p>
                  <p>
                    <span className="label">D</span> To that end, the Library is consulting with prominent publishers of Internet and digital content, attempting to reach agreement on preservation standards and work out deals for archiving information, says Winston Tabb, the associate librarian for library services. In some cases, the Library is discussing with publishers of copyrighted professional journals, news sites and Wedzines, says that it can assist with archiving their content. Currently, publishers can assert their copyright, store all the information themselves and forbid local duplication and storage by libraries and archives.
                  </p>
                  <p>
                    <span className="label">E</span> Vulnerable Data? At the same time, the Library is conducting what may be the first independent scientific tests on the stability of digital storage formats, starting with compact disks, so that it knows how bits can be stored through several in such formats to last. “We don’t see anyone on these doing this kind of study, so it does fill an important niche,” says Marc Roosa, director of preservation for the Library of Congress, which information is important to the Library, the U.S. Copyright Office, and other libraries with growing and aging collections of music CDs, CD-BOMs, DVDs and data stored on writable CDs, that may need to duplicate their collections to save the data.
                  </p>
                  <p>
                    <span className="label">F</span> Experts say the Internet is an even stronger example of the here-today-gone-tomorrow aspect of the digital age. Preservation efforts must begin now, they say, as archivists often cite conventional wisdom that the average life of a Web page is about two months. In recent years, Kahle says, the Internet Archive has taken snapshots of all legally accessible parts of the Internet every two months, so that people in the future will have some idea how it looked and what surfing the Web circa 1996 to 2001 was like. Areas of the Web designated as off limits by site creators, or password protected, are among those not saved during the Internet Archive’s automated sweeps. (Some parts of ABCNEWS.com are forbidden to archivists.)
                  </p>
                  <p>
                    <span className="label">G</span> So far, the archive has collected 40 terabytes of information, about double the amount of digital space it would take to store the Library of Congress’ collection of text-based material, Kahle says. The archive plans to transfer its data to new hard drives every three years so that the information contained is not lost to decaying digital bits. The digital material accessible through his desk “way-back machine” includes a two-terabyte archive of sites related to the 2000 presidential election, frozen as they were and stored periodically as the election campaign and post-election aftermath progressed. Some pages from the 1996 election also are accessible. Most of the pages from both elections are no longer available elsewhere on the Web in their original form.
                  </p>
                  <p>
                    <span className="label">H</span> Museum of E-Failure – An Amateur Effort to Save the Memory of Failed DotComs. However, reconstructing records for the Web gets even harder the farther back in time you go, meaning already there is a loss of historical information. “If somebody were to try to write a dissertation today about the Web in 1994, say, they would be hard-pressed to find the kind of archival primary materials that they’d want,” says Patsy Baudoin, e-journal archiving project manager at the Massachusetts Institute of Technology libraries.
                  </p>
                </div>
              ) : (
              ...
              <div className="question-group">
                ...
                <div className="question question--plain">
                  <div className="question-text">
                    27. Mention of co-operative efforts between archivists and producers of printed materials.
                  </div>
                  {renderDropdown("q27", paragraphOptions)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">
                    28. An outline of the risks now facing archivists and the potential benefits of their work.
                  </div>
                  {renderDropdown("q28", paragraphOptions)}
                </div>
                ...
                <div className="question question--plain">
                  ...
                  {renderDropdown("q29", paragraphOptions)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">30. A similarity between the treatment of internet and older types of media.</div>
                  {renderDropdown("q30", paragraphOptions)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">31. The length of time material usually remains on the internet.</div>
                  {renderDropdown("q31", paragraphOptions)}
                </div>
              </div>

              <div className="question-group" style={{ marginTop: 28 }}>
                <div className="question-group-title">
                  <strong>Questions 32-36</strong>
                </div>
                <p style={{ color: "var(--muted)", fontWeight: 700 }}>
                ...
                <div className="question question--plain">
                  <div className="question-text">
                    32. What is the main difference between the communication of cultural information in the past and now?
                  </div>
                  {renderRadioGroup("q32", q32Options)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">33. Brewster Kahle has set up an archive which is</div>
                  {renderRadioGroup("q33", q33Options)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">34. Laura Campbell wants to establish basic principles to</div>
                  {renderRadioGroup("q34", q34Options)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">35. Marc Roosa refers to research which involves</div>
                  {renderRadioGroup("q35", q35Options)}
                </div>

                <div className="question question--plain">
                  <div className="question-text">36. Patsy Baudoin says researchers could have problems in finding</div>
                  {renderRadioGroup("q36", q36Options)}
                </div>
              </div>

              <div className="question-group" style={{ marginTop: 28 }}>
                <div className="question-group-title">
                  <strong>Questions 37-40</strong>
                </div>
                <p style={{ color: "var(--muted)", fontWeight: 700 }}>
                ...
                <p style={{ color: "var(--text-soft)", lineHeight: 1.55, fontWeight: 700 }}>
                  <strong>Summary</strong>
                  <br />
                  A lot of material is already available, including records of important
                  {renderDropdown("q37", summaryWordOptions, "summary-dropdown")} events. However, some
                  {renderDropdown("q38", summaryWordOptions, "summary-dropdown")} of the internet remain unavailable to
                  {renderDropdown("q39", summaryWordOptions, "summary-dropdown")} people. The data will be copied regularly so that it remains
                  {renderDropdown("q40", summaryWordOptions, "summary-dropdown")} in the future. But information from before 1996 is already almost
                  impossible to find.
                </p>
              </div>
            </div>
