...
function getHlsRootFromHref(href: string): { rootUrl: string; masterFile: string } {
  ...
  return { rootUrl: `${u.origin}${rootPath}`, masterFile: "master.m3u8" };
}
...
function rewritePlaylist(text: string, baseUrl: string, catalog: Catalog, id: string, token: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("#EXT-X-KEY:")) {
      const replaced = line.replace(/URI="([^"]+)"/g, () => {
        return `URI=\"/api/video/key?catalog=${encodeURIComponent(catalog)}&id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}\"`;
      });
      out.push(replaced);
      continue;
    }
    ...
    if (trimmed.startsWith("#EXT-X-MAP:")) {
      const replaced = line.replace(/URI="([^"]+)"/g, (_m, uriVal: string) => {
        const p = String(uriVal || "").trim();
        return `URI=\"/api/video/hls?catalog=${encodeURIComponent(catalog)}&id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}&path=${encodeURIComponent(p)}\"`;
      });
      out.push(replaced);
      continue;
    }
...
export async function GET(request: Request) {
  ...
  let upstream: URL;
  try {
    upstream = new URL(fileToFetch, rootUrl);
    if (!String(upstream.href).startsWith(rootUrl)) {
      throw new Error("bad_path");
    }
  } catch {
    return new NextResponse(JSON.stringify({ error: "invalid_path" }), {
      status: 400,
      headers: noStoreHeaders({ "content-type": "application/json" }),
    });
  }
  ...
  const isPlaylist = fileToFetch.toLowerCase().endsWith(".m3u8") || contentType.includes("application/vnd.apple.mpegurl");

  if (isPlaylist) {
    const txt = await upstreamRes.text();
    const rewritten = rewritePlaylist(txt, rootUrl, catalog, id, token);
